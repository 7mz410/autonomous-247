...
# --- Initialize the Main Orchestrator (cached for performance) ---
@st.cache_resource
...
def initialize_session_state():
    """Sets up default values in the session state if they don't exist."""
    defaults = {
...
def handle_linkedin_auth():
    ...
    if auth_code and not st.session_state.linkedin_authenticated:
        ...
        # Stop further execution this run to prevent UI glitches
        st.stop()
...
if st.session_state.is_generating:
    ...
    content_type = st.selectbox(
        "1. Select a Task:",
        ("YouTube Video", "Astrology Daily Posts", "LinkedIn Post"),
        key="sb_content_type"
    )
        ...
        if st.button(f"🚀 Generate {content_type}", use_container_width=True, type="primary", disabled=(not topic or not niche)):
            ...
            with st.spinner(st.session_state.current_task_message):
                if content_type == "YouTube Video":
                    result = orchestrator.generate_single_youtube_video(
                        topic=topic, niche=niche, auto_search_context=auto_search, upload=False
                    )
...
with st.expander("⚙️ System Automation & Settings"):
    ...
    with st.form("settings_form"):
        ...
        if st.form_submit_button("💾 Save Settings"):
            new_settings = {
                "automation_niche": automation_niche, 
                "upload_days": selected_days, 
                "upload_time": selected_time.strftime("%H:%M")
            }
